...
import { expandHome, normalizePath, validatePath } from './src/utils/path-utils.js';
...
if (args.length === 0) {
  console.error('Usage: mcp-server-filesystem [flags] <allowed-directory> [additional-directories...]');
  process.exit(1);
}
  ...
  try {
    ...
    try {
      ...
      if (realPath !== dir) {
        ...
        if (!allowedDirectories.includes(normalizedRealPath)) allowedDirectories.push(normalizedRealPath);
...
const toolHandlers = {
  ...
  read_multiple_files: (a: unknown) => handleReadMultipleFiles(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  create_file: (a: unknown) => handleCreateFile(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  modify_file: (a: unknown) => handleModifyFile(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  edit_file: (a: unknown) => handleEditFile(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  create_directory: (a: unknown) => handleCreateDirectory(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  list_directory: (a: unknown) => handleListDirectory(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  directory_tree: (a: unknown) => handleDirectoryTree(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  move_file: (a: unknown) => handleMoveFile(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  rename_file: (a: unknown) => handleRenameFile(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  delete_directory: (a: unknown) => handleDeleteDirectory(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  search_files: (a: unknown) => handleSearchFiles(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  find_files_by_extension: (a: unknown) => handleFindFilesByExtension(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  ...
  get_permissions: (a: unknown) => handleGetPermissions(a, permissions, readonlyFlag, noFollowSymlinks, allowedDirectories),
  xml_query: (a: unknown) => handleXmlQuery(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  xml_structure: (a: unknown) => handleXmlStructure(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  xml_to_json: (a: unknown) => handleXmlToJson(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  xml_to_json_string: (a: unknown) => handleXmlToJsonString(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  delete_file: (a: unknown) => handleDeleteFile(a, permissions, allowedDirectories, symlinksMap, noFollowSymlinks),
  json_query: (a: unknown) => handleJsonQuery(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  json_structure: (a: unknown) => handleJsonStructure(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  json_filter: (a: unknown) => handleJsonFilter(a, allowedDirectories, symlinksMap, noFollowSymlinks),
  ...
  json_validate: (a: unknown) => handleJsonValidate(a, allowedDirectories, symlinksMap, noFollowSymlinks),
...
const tools = !permissions.fullAccess
  ? allTools.filter(t => {
      ...
      if (permissions.create && ['create_file', 'create_directory', 'xml_to_json'].includes(t.name)) return true;
      if (permissions.edit && ['modify_file', 'edit_file'].includes(t.name)) return true;
      if (permissions.move && t.name === 'move_file') return true;
      if (permissions.rename && t.name === 'rename_file') return true;
      if (permissions.delete && ['delete_file', 'delete_directory'].includes(t.name)) return true;
      return false;
    })
  : allTools;
...
async function runServer() {
  if (useHttp) {
    ...
    console.error(`Secure MCP Filesystem Server running on HTTP stream port ${port}`);
  } else {
